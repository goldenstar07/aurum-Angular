...
export class ActivityChatService {
  ...
  getMessages() {
    this.messagesCol = this.afs.collection("messages", ref =>
      ref.orderBy("date", "desc")
    );
    return this.messagesCol.snapshotChanges().map(actions => {
  ...
  deleteAllMessageByHotelId(hotelId) {
    this.afs
      .collection("messages", ref => ref.where("htId", "==", hotelId))
      .snapshotChanges()
      .forEach(docs => {
        docs.forEach(_doc => {
          this.afs.doc(`messages/${_doc.payload.doc.id}`).delete();
        });
      });
  }
}
