...
export class InspectionComponent extends InspectionManager implements OnInit {
  ...
  ngOnInit() {
    this.inventoryService.getInventories().subscribe(res => {
     ...
     if(this.inventoryItems.inspection) {
        this.getDates(this.inventoryItems.inspection[Object.keys(this.inventoryItems.inspection)[0]]); 
  ...
  saveFormInput() {
    ...
    this.form.value.inventories.forEach(item => {
      if (!this.inventoryItems.inspection[item.item]) this.addNewItem(item.item);
    });
    ...
    this.form.value.inventories.forEach(item => {
      ...
      this.inventoryItems.inspection[item.item].data[indexOfItem].cleaning = item.cleaning;
      this.inventoryItems.inspection[item.item].data[indexOfItem].inspector = item.inspector;
      this.inventoryItems.inspection[item.item].data[indexOfItem].comments = item.comments;      
    });
...
updateItem() {
  ...
  sortByDate() {
    for (let key in this.inventoryItems.inspection) {
      this.inventoryItems.inspection[key].data.sort((a, b) => +new Date(b.date) - +new Date(a.date));
    }
    this.getDates(this.inventoryItems.inspection[Object.keys(this.inventoryItems.inspection)[0]]);
  }
